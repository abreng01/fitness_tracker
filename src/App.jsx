...
// ── Athlete DNA ───────────────────────────────────────────────────────────────
// A lifetime training character score (0-100) based on four signals:
// Discipline, Intensity, Durability, Resilience — all computed from full history.
function computeAthleteDNA(member, logs){
  const today = todayStr();
  const acts = member.activities || [];
  const sd = member.startDate || null;
  if(acts.length === 0) return null;
  ...
  const firstDate = sd || sortedDates[0];
  const totalDaysSinceStart = Math.max(1,
    Math.round((new Date(today+"T00:00:00") - new Date(firstDate+"T00:00:00")) / 86400000) + 1
  );

  // Helper: status of a day
  ...
  function daySkipped(dateStr){
    return acts.some(a => {
      const l = getActivityLogs(logs, member.id, a.id)[dateStr];
      return l && l.status === "skipped";
    });
  }
  function dayAboveTarget(dateStr){
    return acts.some(a => {
      const al = getActivityLogs(logs, member.id, a.id);
      const l = al[dateStr];
  ...
  // 1. DISCIPLINE — lifetime consistency: done / (done + skipped)
  let doneDays = 0, skippedDays = 0;
  for(const d of sortedDates){
    if(dayDone(d)) doneDays++;
    else if(daySkipped(d)) skippedDays++;
  }
  const activeDays = doneDays + skippedDays;
  const discipline = activeDays > 0 ? doneDays / activeDays : doneDays / totalDaysSinceStart;

  // 2. INTENSITY — % of done days that were above target, all-time
  ...
  let recoveryScores = [];
  let inStreak = false, missStart = null;
  for(let i=0; i<sortedDates.length; i++){
    const d = sortedDates[i];
    if(dayDone(d)){
      if(missStart !== null){
        const gap = Math.round((new Date(d+"T00:00:00") - new Date(missStart+"T00:00:00")) / 86400000);
        recoveryScores.push(Math.max(0, 1 - (gap-1)/7)); // 1 day gap=1.0, 8+ days=0
        missStart = null;
      }
      inStreak = true;
    } else if(daySkipped(d)){
      if(inStreak && missStart === null) missStart = d;
      inStreak = false;
    }
  }
  const resilience = recoveryScores.length > 0
    ? recoveryScores.reduce((s,v)=>s+v,0) / recoveryScores.length
  ...
  // Archetype
  const archetypes = [
    {min:85, name:"The Machine",    icon:"⚙️",  color:"#1565C0", desc:"Relentless, consistent, never misses"},
    {min:70, name:"The Grinder",    icon:"💪",  color:"#2E7D32", desc:"High volume, pushes hard, occasional gaps"},
    {min:55, name:"The Streaker",   icon:"🔥",  color:"#E65100", desc:"Runs hot then needs recovery"},
    {min:40, name:"The Sprinter",   icon:"⚡",  color:"#6A1B9A", desc:"Intense bursts, inconsistent baseline"},
    {min:25, name:"The Restarter",  icon:"🔄",  color:"#AD1457", desc:"Good intentions, keeps rebuilding"},
    {min:0,  name:"The Beginner",   icon:"🌅",  color:"#546E7A", desc:"Still finding the rhythm"},
  ];
  ...
  return {score, archetype, signals, strongest, weakest, longestStreak, doneDays};
}

function AthleteDNACard({member, logs}){
  const d = computeAthleteDNA(member, logs);
  const [expanded, setExpanded] = useState(false);
  if(!d) return null;

  const {score, archetype, signals, strongest, weakest} = d;
  const barFill = score; // 0-100
  ...
  return <div style={{background:C.surface,border:`1.5px solid ${C.border}`,borderRadius:16,marginBottom:12,overflow:"hidden"}}>
    ...
    <div onClick={()=>setExpanded(e=>!e)} style={{padding:"14px 16px",cursor:"pointer",
      ...
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between"}}>
        <div>
          <div style={{fontSize:10,fontWeight:700,color:C.muted,letterSpacing:0.5,marginBottom:4}}>🧬 ATHLETE DNA</div>
          <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:6}}>
            <span style={{fontSize:22}}>{archetype.icon}</span>
            <div>
              <div style={{fontSize:16,fontWeight:900,color:archetype.color}}>{archetype.name}</div>
              <div style={{fontSize:11,color:C.muted}}>{archetype.desc}</div>
            </div>
          </div>
          {/* Score bar */}
          <div style={{display:"flex",alignItems:"center",gap:8}}>
            <div style={{flex:1,height:6,borderRadius:99,background:C.border,overflow:"hidden"}}>
              <div style={{height:"100%",width:`${barFill}%`,background:archetype.color,
                borderRadius:99,transition:"width 0.6s ease"}}/>
            </div>
            <span style={{fontSize:12,fontWeight:800,color:archetype.color,minWidth:32}}>{score}</span>
          </div>
        </div>
        <span style={{color:C.muted,fontSize:14,marginLeft:12}}>{expanded?"▾":"▸"}</span>
      </div>
    </div>

    {expanded&&<div style={{padding:"14px 16px",display:"flex",flexDirection:"column",gap:12}}>

      {/* Four signal bars */}
      <div style={{display:"flex",flexDirection:"column",gap:8}}>
        <div style={{fontSize:10,fontWeight:700,color:C.muted,letterSpacing:0.5,marginBottom:2}}>SIGNALS</div>
        {signals.map(s=>{
          const pct = s.value;
          const color = pct>=70?"#4CAF50":pct>=45?"#F9A825":"#FF7043";
          return <div key={s.label}>
            <div style={{display:"flex",justifyContent:"space-between",marginBottom:3}}>
              <span style={{fontSize:12,fontWeight:600,color:C.text}}>{s.label} <span style={{fontSize:10,color:C.muted,fontWeight:400}}>{s.weight}</span></span>
      ...
      {/* Strongest / weakest */}
      <div style={{display:"flex",gap:8}}>
        <div style={{flex:1,background:"rgba(76,175,80,0.08)",border:"1px solid rgba(76,175,80,0.3)",
          borderRadius:10,padding:"10px 12px"}}>
          <div style={{fontSize:10,fontWeight:700,color:"#2E7D32",marginBottom:2}}>💪 STRENGTH</div>
          <div style={{fontSize:13,fontWeight:700,color:C.text}}>{strongest.label}</div>
          <div style={{fontSize:11,color:"#4CAF50"}}>{strongest.value}%</div>
        </div>
        <div style={{flex:1,background:"rgba(255,152,0,0.08)",border:"1px solid rgba(255,152,0,0.3)",
          borderRadius:10,padding:"10px 12px"}}>
          <div style={{fontSize:10,fontWeight:700,color:"#E65100",marginBottom:2}}>⚠️ GAP</div>
          <div style={{fontSize:13,fontWeight:700,color:C.text}}>{weakest.label}</div>
          <div style={{fontSize:11,color:"#FF9800"}}>{weakest.value}%</div>
        </div>
      </div>

      {/* Legend — all archetypes */}
      <div style={{background:C.bg,borderRadius:10,padding:"12px 14px"}}>
        <div style={{fontSize:10,fontWeight:700,color:C.muted,letterSpacing:0.5,marginBottom:8}}>ARCHETYPES</div>
        <div style={{display:"flex",flexDirection:"column",gap:5}}>
          {[
            {min:85,name:"The Machine",   icon:"⚙️", color:"#1565C0"},
            {min:70,name:"The Grinder",   icon:"💪", color:"#2E7D32"},
            {min:55,name:"The Streaker",  icon:"🔥", color:"#E65100"},
            {min:40,name:"The Sprinter",  icon:"⚡", color:"#6A1B9A"},
            {min:25,name:"The Restarter", icon:"🔄", color:"#AD1457"},
            {min:0, name:"The Beginner",  icon:"🌅", color:"#546E7A"},
          ].map(a=>{
            const isCurrent = score >= a.min && (a.min === 85 ? score >= 85 : score < a.min + 15 || a.min === 0);
            const isActive = score >= a.min && archetype.name === a.name;
            return <div key={a.name} style={{
              ...
              <div style={{display:"flex",alignItems:"center",gap:8}}>
                ...
                <span style={{fontSize:12,fontWeight:isActive?700:400,color:isActive?a.color:C.muted}}>{a.name}</span>
              </div>
              <span style={{fontSize:10,color:C.muted}}>{a.min}+</span>
            </div>;
          })}
        </div>
      </div>
...
function MemberCard({member,logs,allMembers,onLogAll,onEggChange,onEdit,onNewBadge,year,month,theme,onOpenPP,onGrowthSave,onGkSave,onBraverySave,onBraveryDelete,onBraveryUpdate,onIllnessSave,onIllnessDelete,onOlympiadSave,onOlympiadDelete,onOlympiadUpdate,onDeleteEntry}){
  ...
  return <div
    ...
    {/* Athlete DNA */}
    <AthleteDNACard member={member} logs={logs}/>
